...
import pickle  # Pickle supports by default a textual protocol, but has also a binary protocol,
# which is more efficient, but not human-readable
import os
...
class AddressBook(object):
    ...
    def __str__(self):
        return '[Name: {0} | Address: {1} | Email: {2} | Phone: {3}]'.format(self.name, self.address, self.email,
                                                                             self.phone)
